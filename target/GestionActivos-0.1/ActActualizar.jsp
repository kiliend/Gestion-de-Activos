<%@page import="Modelos.GestionActivos"%>
<%@page import="Modelos.GestionActivosDAO"%>
...
<head>
    ...
    <title>Actualizar Activo - Gestion Activos</title>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Gestión de Activos</h1>
                            <ol class="breadcrumb mb-4">
                                <li class="breadcrumb-item active">Actualizar Activo</li>
                            </ol>

                        
<body>
       
    <!-- Formulario para buscar cliente y listar sus activos -->
        <form action="GestionActivosServlet" method="GET" class="mb-4">
            <div class="row">
                <div class="col-md-6">
                    <label for="idCliente" class="form-label">Buscar Cliente (ID):</label>
        ...
        <!-- Lista de activos -->
        <h3>Lista de Activos del Cliente</h3>
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <%
                        // Obtener ID del cliente desde la petición
                        int idCliente = request.getParameter("idCliente") != null ? Integer.parseInt(request.getParameter("idCliente")) : 0;
                        if (idCliente > 0) {
                            // Crear el DAO y listar los activos del cliente
                            GestionActivosDAO dao = new GestionActivosDAO();
                            List<GestionActivos> activos = dao.listarActivos(idCliente);
                            if (activos != null && !activos.isEmpty()) {
                                // Iterar sobre los activos y mostrarlos en la tabla
                                for (GestionActivos activo : activos) {
                    %>
                    <tr>
                        <td><%= activo.getIdGestion() %></td>
                        <td><%= activo.getIdEncargado() %></td>
                        <td><%= activo.getIdOperarios() %></td>
                        <td><%= activo.getIdFactura() %></td>
                        <td><%= activo.getFecha() %></td>
                        <td><%= activo.getSituacion() %></td>
                        <td class="btn-actions">
                            <!-- Botón de Eliminar -->
                            <a href="GestionActivosServlet?accion=eliminar&idGestion=<%= activo.getIdGestion() %>" class="btn btn-danger btn-sm">Eliminar</a>
                            <!-- Botón de Modificar -->
                            <a href="GestionActivosServlet?accion=editar&idGestion=<%= activo.getIdGestion() %>" class="btn btn-warning btn-sm">Modificar</a>
                        </td>
                    </tr>
                    <%
                                }
                    ...
                    %>
                    <tr>
                        <td colspan="7" class="text-center">No se encontraron activos para este cliente.</td>
                    </tr>
                    <%
                            }
                        }
