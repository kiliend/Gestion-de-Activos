...
<head>
    ...
    <title>Registrar Usuario - Gestion Activos</title>
    ...
    <div id="layoutSidenav_nav">
       ...
       <div class="card mb-4">
           <div class="card-header">
               <i class="fas fa-table me-1"></i>Formulario de Registro/Actualización de Usuario
           </div>
           <div class="card-body">
               <!-- Aquí comienza el formulario -->
               <form action="UsuarioServlet" method="POST">
                   <!-- Campo oculto para identificar si es una actualización -->
                   <input type="hidden" name="accion" value="<%= request.getAttribute("accion") != null ? request.getAttribute("accion") : "registrar" %>" />

                   <!-- Si es actualización, le pasas el id del usuario -->
                   <input type="hidden" name="id" value="<%= request.getAttribute("usuario") != null ? ((Usuario)request.getAttribute("usuario")).getIdUsuario() : "" %>" />

                   <div class="mb-3">
                       <label for="nombre" class="form-label">Nombre</label>
                       <input type="text" class="form-control" id="nombre" name="nombre" value="<%= request.getAttribute("usuario") != null ? ((Usuario)request.getAttribute("usuario")).getNombre() : "" %>" required>
                   </div>
                   <div class="mb-3">
                       <label for="email" class="form-label">Correo Electrónico</label>
                       <input type="email" class="form-control" id="email" name="email" value="<%= request.getAttribute("usuario") != null ? ((Usuario)request.getAttribute("usuario")).getEmail() : "" %>" required>
                   </div>
                   <div class="mb-3">
                   ...
                   <div class="mb-3">
                       ...
                       <select class="form-select" id="rol" name="rol" required>
                           <option value="administrador" <%= request.getAttribute("usuario") != null && ((Usuario)request.getAttribute("usuario")).getRol().equals("administrador") ? "selected" : "" %>>Administrador</option>
                           <option value="operador" <%= request.getAttribute("usuario") != null && ((Usuario)request.getAttribute("usuario")).getRol().equals("operador") ? "selected" : "" %>>Operador</option>
                           <option value="ventas" <%= request.getAttribute("usuario") != null && ((Usuario)request.getAttribute("usuario")).getRol().equals("ventas") ? "selected" : "" %>>Ventas</option>
                       </select>
                   </div>
                   <div class="mb-3">
                       <label for="estado" class="form-label">Estado</label>
                       <select class="form-select" id="estado" name="estado" required>
                           <option value="activo" <%= request.getAttribute("usuario") != null && ((Usuario)request.getAttribute("usuario")).getEstado().equals("activo") ? "selected" : "" %>>Activo</option>
                           <option value="inactivo" <%= request.getAttribute("usuario") != null && ((Usuario)request.getAttribute("usuario")).getEstado().equals("inactivo") ? "selected" : "" %>>Inactivo</option>
                       </select>
                   </div>
                   <button type="submit" class="btn btn-primary"><%= request.getAttribute("accion") != null && request.getAttribute("accion").equals("actualizar") ? "Actualizar" : "Registrar" %></button>
               </form>
           </div>
       </div>
