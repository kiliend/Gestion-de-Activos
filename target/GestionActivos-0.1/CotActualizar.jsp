...
<head>
    ...
    <title>Actualizar Cotización - Gestion Activos</title>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Gestión de Cotizaciones</h1>
                            <ol class="breadcrumb mb-4">
                                <li class="breadcrumb-item active">Actualizar Cotización</li>
                            </ol>
                        
  <div class="container mt-4">
        <h2>Actualizar Cotización</h2>
        
        <!-- Barra de búsqueda por ID de cotización -->
        <form action="CotizacionServlet" method="GET">
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Buscar por ID de cotización" name="cotizacionId" required>
                <button class="btn btn-primary" type="submit">Buscar</button>
            </div>
        </form>

        <!-- Formulario para actualizar cotización (si la cotización está encontrada) -->
        <c:if test="${not empty cotizacion}">
            <form action="CotizacionServlet" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="cliente" name="cliente" value="${cotizacion.clienteNombre}" required>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="total" name="total" value="${cotizacion.total}" required>
                </div>
                <div class="mb-3">
                    <label for="estado" class="form-label">Estado</label>
                    <select class="form-select" id="estado" name="estado" required>
                        <option value="pendiente" <c:if test="${cotizacion.estado == 'pendiente'}">selected</c:if>>Pendiente</option>
                        <option value="aceptada" <c:if test="${cotizacion.estado == 'aceptada'}">selected</c:if>>Aceptada</option>
                        <option value="rechazada" <c:if test="${cotizacion.estado == 'rechazada'}">selected</c:if>>Rechazada</option>
                    </select>
                </div>
